...
from utils.memmap import MemmapArray
from utils.utils import NUMPY_TO_TORCH_DTYPE_DICT
...
class PERBuffer(ReplayBuffer):
    ...
    Stores priorities for each transition and samples according to priority.
    Returns importance-sampling weights and indices for updating priorities.
    """
    def __init__(
        self,
        buffer_size: int,
        ...
        alpha: float = 0.6,
        beta: float = 0.4,
        beta_increment_per_sampling: float = 1e-6,
        epsilon: float = 1e-6,
        **kwargs,
    ):
        super().__init__(buffer_size, n_envs, obs_keys, memmap, memmap_dir, memmap_mode, **kwargs)
        ...
        self.priorities = np.zeros((self._buffer_size,), dtype=np.float32)
        self.max_priority = 1.0

    def add(self, data: "ReplayBuffer" | Dict[str, np.ndarray], validate_args: bool = False) -> None:
        prev_pos = self._pos
        prev_full = self._full
        super().add(data, validate_args)
        # Set max priority for new samples
        data_len = next(iter(data.values())).shape[0]
        next_pos = (prev_pos + data_len) % self._buffer_size
        if next_pos <= prev_pos or (data_len > self._buffer_size and not prev_full):
            idxes = np.array(list(range(prev_pos, self._buffer_size)) + list(range(0, next_pos)))
        else:
            idxes = np.array(range(prev_pos, next_pos))
        self.priorities[idxes] = self.max_priority

    def sample(self, batch_size: int, sample_next_obs: bool = False, clone: bool = False, n_samples: int = 1, **kwargs):
        if not self._full and self._pos == 0:
            raise ValueError("No sample has been added to the buffer. Please add at least one sample calling 'self.add()'")
        if self._full:
            valid_idxes = np.arange(self._buffer_size)
        else:
            valid_idxes = np.arange(self._pos)
        probs = self.priorities[valid_idxes] ** self.alpha
        probs /= probs.sum() + 1e-8
        batch_idxes = np.random.choice(valid_idxes, batch_size * n_samples, p=probs)
        # Importance-sampling weights
        total = len(valid_idxes)
        weights = (total * probs[batch_idxes]) ** (-self.beta)
        weights /= weights.max() + 1e-8
        self.beta = min(1.0, self.beta + self.beta_increment_per_sampling)
        samples = self._get_samples(batch_idxes, sample_next_obs=sample_next_obs, clone=clone)
        # Reshape to [n_samples, batch_size, ...]
        samples = {k: v.reshape(n_samples, batch_size, *v.shape[1:]) for k, v in samples.items()}
        return samples, batch_idxes, weights.reshape(n_samples, batch_size)
