...
from algos.ppo.loss import entropy_loss, policy_loss, value_loss
from algos.ppo_recurrent.agent import RecurrentPPOAgent, build_agent
from algos.ppo_recurrent.utils import prepare_obs, test
...
from utils.utils import gae, normalize_tensor, polynomial_decay, save_configs
...
def main(fabric: Fabric, cfg: Dict[str, Any]):
    ...
    if cfg.env.num_envs == 1:
        # Initialize a single environment without vectorization
        envs = make_env(
            cfg,
            cfg.seed,
            0,  # rank
        ...
        # envs.single_reward_range = envs.reward_ranges
    else:
        # Use vectorized environment for multiple environments
        ...
        with torch.inference_mode():
            for _ in range(0, cfg.algo.rollout_steps):
                ...
                with timer("Time/env_interaction_time", SumMetric, sync_on_compute=False):
                    ...
                    if terminated:
                        fabric.log("Survival Steps/Episode", episode_survival_step, episode_num)
                        envs.reset()
                        episode_num += 1
                        episode_survival_step = 0
    ...
    if not cfg.model_manager.disabled and fabric.is_global_zero:
        from algos.ppo.utils import log_models
        from utils.mlflow import register_model
