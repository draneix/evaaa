...
def gae(
    ...
    for t in reversed(range(num_steps)):
        ...
        advantages[t] = lastgaelam = (
            delta + nextnonterminal * lastgaelam * gamma * gae_lambda
        )
...
def normalize_tensor(
    tensor: Tensor, eps: float = 1e-8, mask: Optional[Tensor] = None
) -> Tensor:
...
def two_hot_encoder(
    tensor: Tensor, support_range: int = 300, num_buckets: Optional[int] = None
) -> Tensor:
    ...
    buckets = torch.linspace(
        -support_range, support_range, num_buckets, device=tensor.device
    )
...
def two_hot_decoder(tensor: torch.Tensor, support_range: int) -> torch.Tensor:
    ...
    if num_buckets % 2 == 0:
        raise ValueError("support_size must be odd")
    support = torch.linspace(-support_range, support_range, num_buckets).to(
        tensor.device
    )
...
class Ratio:
    ...
    def __init__(self, ratio: float, pretrain_steps: int = 0):
        if pretrain_steps < 0:
            raise ValueError(
                f"'pretrain_steps' must be non-negative, got {pretrain_steps}"
            )
        if ratio < 0:
