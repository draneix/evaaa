...
using Unity.MLAgents; // For the Academy class
//using Unity.MLAgents.Exceptions; // For the UnityAgentsException class
...
public class TrainingAreaReplicator : MonoBehaviour
{
    /// <summary>
        ...
        void AddEnvironments()
        {
            ...
            {
                throw new System.Exception("The number of training areas that you have specified exceeds the size of the grid.");
            }
                ...
                for (int y = 0; y < m_GridSize.y; y++)
                {
                    for (int x = 0; x < m_GridSize.x; x++)
                    {
                        if (m_AreaCount == 0)
                        {
                            ...
                            m_AreaCount = 1;
                            Debug.Log("ResetResourceArea called for: baseArea");
                            // // Get the Field component and call ResetResourceArea for the baseArea
                            // Field baseFieldComponent = baseArea.GetComponent<Field>();
                            // if (baseFieldComponent != null)
                            // {
                            //     baseFieldComponent.ResetResourceArea(baseArea);
                            //     Debug.Log("ResetResourceArea called for: " + baseArea.name);
                            // }
                            // else
                            // {
                            //     Debug.LogWarning("Field component not found on the base area.");
                            // }
                        }
                        else if (m_AreaCount < numAreas)
                        {
                            ...
                            // Call ResetResourceArea for the new area
                            var fieldComponent = area.GetComponent<Field>();
                            if (fieldComponent != null)
                            {
                                fieldComponent.ResetResourceArea(area);
                                Debug.Log("ResetResourceArea called for: " + area.name);
                            }
                            else
                            {
                                Debug.LogError("Field component not found in: " + area.name);
                            }
